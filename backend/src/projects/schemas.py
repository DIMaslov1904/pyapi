...
class CmsSystemBase(BaseModel):
    ...
    base_admin_url: str | None  = Field(None, min_length=2, max_length=120, description='Стандартный путь до админ панели')
...
class ProjectExtensionBase(BaseModel):
    ...
    git: str | None = Field(None, min_length=11, max_length=120, description='URL адрес к Git-репозиторию')
    figma: str | None = Field(None, min_length=11, max_length=120, description='URL адрес к Figma')
    cms_system_id: int | None = Field(None, description='id CMS системы')
    custom_admin_url: str | None = Field(None, min_length=2, max_length=120, description='Не стандартный URL адрес до админ панели')
...
class ProjectBase(BaseModel):
    ...
    url: str = Field(..., min_length=11, max_length=120, description='URL адрес проекта')
    description: str | None = Field(None, max_length=5000, description='Описание проекта')
    document: str = Field(..., min_length=11, max_length=120, description='URL адрес к документации')
...
class ProjectResponse(ProjectBase):
    ...
    extensions: ProjectExtensionResponse = Field(..., description='Расширенная информация по проекту')
